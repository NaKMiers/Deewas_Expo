    ...
    <>
      ...
      <SafeAreaView className='flex-1'>
        ...
        <View className='flex flex-row items-center justify-end px-21/2 py-21/2 md:px-21'>
          ...
          >
            <SelectTrigger className='border-transparent bg-transparent' style={{ height: 36 }}>
              <Text>{selectedLanguage.label}</Text>
            </SelectTrigger>

            <SelectContent className='border-transparent bg-secondary shadow-none'>
              {languages.map((item, index) => (
                <SelectItem value={item.value} label={item.label} key={index} />
              ))}
            </SelectContent>
          </Select>
        </View>
        ...
        <Separator className='h-0 my-2.5' />
      </SafeAreaView>
    </>
  )
}
