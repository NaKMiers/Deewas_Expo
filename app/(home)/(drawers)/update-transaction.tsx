...
function UpdateTransactionPage() {
  ...
  const transaction = useAppSelector(state => state.screen.transactionToEdit)
  ...
  const handleUpdateTransaction: SubmitHandler<FieldValues> = useCallback(
    ...
    [dispatch, validate, tError, tSuccess, transaction, locale]
  )
          ...
          <View className="mx-auto w-full max-w-[500px] flex-1 p-21">
            <View>
              <Text className="text-center text-xl font-semibold text-primary">
                {t('Update') + ' '}
                {transaction?.type && (
                  <Text className={cn(checkTranType(transaction.type).color)}>
                    {t(transaction.type)}
                  </Text>
                )}
                {' ' + t('transaction')}
            ...
            <View className="mt-6 flex flex-col gap-6">
              ...
              <View className="flex flex-1 flex-col">
                <Text
                  className={cn('mb-1 font-semibold', errors.categoryId?.message && 'text-rose-500')}
                >
                  {t('Category')}
                </Text>
                <TouchableOpacity
