...
import { setInitCategory, setInitWallet, setSelectedCategory } from '@/lib/reducers/screenReducer'
...
function CreateTransactionPage() {
  ...
  const { initWallet, initCategory } = useAppSelector(state => state.screen)
  ...
  const defaultValues = useMemo(
    ...
    [curWallet, initCategory, initDate, initWallet, type]
  )
  ...
  useEffect(() => {
    if (initDate) {
      setValue('date', moment(initDate).format('YYYY-MM-DD'))
    }
  }, [initDate, setValue])
        ...
        <ScrollView className="flex-1">
          <View className="mx-auto w-full max-w-[500px] flex-1 p-21">
            ...
            <View className="mt-6 flex-col gap-6">
              ...
              <View>
                <Text
                  className={cn('mb-1.5 font-semibold', errors.walletId?.message && 'text-rose-500')}
                >
                  {t('Wallet')}
                </Text>
                <WalletPicker
                  ...
                  wallet={
                    (form.walletId && wallets.find(w => w._id === form.walletId)) ||
                    initWallet ||
                    curWallet
                  }
