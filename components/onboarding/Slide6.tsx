...
export default function Slide6({ onPress }: { onPress: () => void }) {
  ...
  return (
    <View className='mx-auto flex w-full max-w-[500px] flex-1 items-center justify-center'>
      <View className='w-full overflow-hidden rounded-lg shadow-lg' style={{ height: 200 }}>
        <Image source={images.onboardingWelcome} resizeMode='cover' className='h-full w-full' />
      </View>
      ...
      <View className='w-full flex-1'>
        ...
        >
          ...
          <View className='rounded-full bg-secondary p-1'>
            <Icon render={LucideArrowRight} size={18} />
          </View>
        </TouchableOpacity>
      </View>
    </View>
