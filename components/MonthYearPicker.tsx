  ...
  return (
    <Popover>
      ...
      <PopoverTrigger asChild>
        <Button variant='outline' className='flex flex-row items-center gap-2 py-0'>
          <Icon render={LucideCalendarFold} size={20} />
          <Text className='font-semibold capitalize'>
            {format(currentMonth, 'MMMM yyyy', { locale: getLocale(locale) })}
          </Text>
          <Icon render={LucideChevronDown} size={20} />
        </Button>
      </PopoverTrigger>
          ...
          <View className='flex flex-col gap-2'>
            ...
            >
              <SelectTrigger className='capitalize'>
                <SelectValue className='text-lg text-primary' placeholder='Select month' />
              </SelectTrigger>
              <SelectContent>
                <ScrollView>
