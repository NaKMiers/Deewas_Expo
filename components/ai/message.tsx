...
export default function Message({ role, content, parts }: MessageProps) {
  ...
  if (toolInvocations) {
    ...
    if (error) {
      return <BasicMessage role={role} content={error} />
    }
    ...
    switch (toolName) {
      case 'get_all_wallets': {
        ...
        if (wallets.length <= 0) {
          return <BasicMessage role={role} content='No wallets found' />
        }

        // show a list of wallets
        return (
          <View className='flex flex-col'>
            {message && <BasicMessage content={message} role={role} />}
            <FlatList
              horizontal
              data={wallets}
      ...
      case 'update_wallet': {
        ...
        if (!wallet) {
          return <BasicMessage role={role} content={'No wallet found'} />
        }
        ...
        return (
          <View className='flex flex-col'>
            ...
            <WalletCard wallet={wallet} className={cn('mb-21/2', !message && 'mt-21/2')} />
          </View>
        )
      }
      case 'delete_wallet': {
        ...
        // show the message
        return <BasicMessage role={role} content={`Wallet "${wallet.name}" deleted successfully!`} />
      }
      case 'transfer_fund_from_wallet_to_wallet': {
        ...
        if (!sourceWallet) {
          return <BasicMessage role={role} content={'No source wallet found'} />
        }

        // check if destination wallet was found
        if (!destinationWallet) {
          return <BasicMessage role={role} content={'No destination wallet found'} />
        }
      ...
      case 'get_all_categories': {
        ...
        if (categories.length <= 0) {
          return <BasicMessage role={role} content='No categories found' />
        }
      ...
      case 'update_category': {
        ...
        if (!category) {
          return <BasicMessage role={role} content={'No category found'} />
        }
        ...
        return (
          <View className='flex flex-col'>
            ...
            <Category category={category} className={cn('mb-21/2', !message && 'mt-21/2')} />
          </View>
        )
      }
      case 'delete_category': {
        ...
        // show the message
        return <BasicMessage role={role} content={`Category "${category.name}" deleted successfully!`} />
      }
      case 'get_budgets': {
        ...
        if (budgets.length <= 0) {
          return <BasicMessage role={role} content='No budgets found' />
        }
      ...
      case 'create_budget': {
        ...
        if (!budget) {
          return <BasicMessage role={role} content={'No budget found'} />
        }
      ...
      case 'get_all_transactions': {
        ...
        if (transactions.length <= 0) {
          return <BasicMessage role={role} content='No transactions found' />
        }
      ...
      case 'update_transaction': {
        ...
        if (!transaction) {
          return <BasicMessage role={role} content={'No transaction found'} />
        }
        ...
        return (
          <View className='flex flex-col'>
            ...
            <Transaction transaction={transaction} className={cn('mb-21/2', !message && 'mt-21/2')} />
          </View>
        )
      }
  ...
  return <BasicMessage role={role} content={content} />
}
